...
> - 不可能所有进程都处于就绪态
> - 在分时操作系统中，**就绪态进程的数量通常多于阻塞态进程**
>   - **分时系统的核心目标：快速响应交互式任务**
>     - **设计初衷**：让多个用户/任务**公平共享CPU**，每个任务都能获得“看似独占”的体验。
>     - **关键需求**：高频率的进程切换（时间片轮转），确保所有交互式任务（如命令行、编辑器）能及时响应。
>     - **结果**：进程因时间片耗尽频繁进入就绪态，而非长期阻塞。
>   - **运行 → 就绪**：时间片耗尽（被动）
>   - **运行 → 阻塞**：主动等待资源（如I/O）
>   - **阻塞 → 就绪**：资源可用后被唤醒（如I/O完成中断）
...
> 在进程的整个生命周期中，系统总是通过其 PCB 对进程进行控制。 PCB 是进程存在的唯一标志，PCB **常驻内存**。
...
1. **父进程调用 `fork()`**  
   ...
   - **关键行为**：  
     - 子进程与父进程 **并发执行**，操作系统通过 **写时复制（Copy-on-Write）** 优化内存复制效率。
       ...
       > - 并发进程的运行结果具有**不可再现性**。
       > - 并发进程执行的相对速度与进程调度策略有关，分配的处理机的数量以及时间长短，都会影响进程执行的相对速度。
       > - 一个进程可以**顺序地**执行一个或多个程序，只要执行过程中改变其 CPU 状态和内存空间即可。但不能**同时执行**多个程序。
...
1. **`fork()` 的作用**：
   ...
   - 但是注意父子进程共享文件表，父子进程享有相同的文件偏移量。但如果父子进程的文件描述符是不一样的，所以虽然两个文件描述符都是对同一个文件进行操作，但是此时父子进程不享有相同的文件偏移量，各自的文件偏移量互不影响。
...
>    - **调度层级**：属于 **长期调度（Long-term Scheduling）**，负责从**外存**的作业队列中选择作业加载到**内存**。
...
>    - **中期调度**：通过交换（Swapping）技术将部分**进程**暂时移出**内存**，平衡系统负载。
...
Thread ，进程的轻型实体，也叫“轻量级进程”，是一系列活动按事先设定好的顺序依次执行的过程，是一系列指令的集合，是一条执行路径，**不能单独存在，必须包含在进程中。线程，** 是 OS 中 **运算调度的最小单位。**因此线程包含 CPU 现场，可以独立执行程序。

> 但**进程**依然是资源配置的单位。内核级线程是处理器调度和分配的单位。

轻型实体、独立调度和分派的基本单位、可并发执行、共享进程资源，即线程没有自己独立的地址空间，要共享所属进程的地址空间。
...
- 线程的创建是轻量级的，而进程的创建是重量级的（进程的切换是需要 **调度器** 来完成的。线程的切换则可以直接由 **用户程序** 来完成。）
  ...
  > #### 系统级线程（内核级线程，Kernel-Level Threads, KLT）
  >
  > - 切换必须由内核参与：
  >   - 线程控制块（TCB）由内核管理，切换时需要更新内核数据结构（如寄存器状态、页表）。
  >   - 通过 **系统调用（如 `sched_yield()`）** 或 **中断** 触发调度。
  >   - **硬件支持**：依赖 CPU 的上下文切换机制（如 x86 的 `TSS` 或现代操作软件的软件上下文切换）
  >
  > #### 用户级线程（User-Level Threads, ULT）
  >
  > - 切换无需内核支持：
  >   - 线程管理完全在用户空间实现（如协程、轻量级线程库）。
  >   - 切换仅需修改用户态栈指针和寄存器，**不涉及内核态切换**。
  >   - 限制：
  >     - **多对一线程模型**中，一个用户线程阻塞（如 I/O）会导致整个进程阻塞（因为内核不知道用户线程的存在）。
  >     - 无法利用多核 CPU（除非结合内核线程）。
...
> ### 多对一线程模型（Many-to-One Model）的阻塞问题详解
>
> 多对一线程模型的核心特点是 **多个用户级线程（ULT）绑定到单个内核级线程（KLT）**。这种设计虽然轻量高效，但存在严重的阻塞问题，以下是其原理和影响的分析：
>
> #### 1. 阻塞问题的本质
>
> 在多对一模型中，**所有用户线程共享同一个内核线程**。这意味着：
>
> - **内核视角**：操作系统只能看到一个内核线程（即一个调度单位），无法感知内部的用户线程。
> - **用户线程的阻塞操作**（如 I/O、锁等待）会通过系统调用传递到内核线程，导致内核线程进入阻塞状态。
> - **连锁反应**：由于所有用户线程依赖唯一的内核线程，**内核线程阻塞 = 整个进程阻塞**，所有用户线程停止执行。
>
> #### 2. 阻塞发生的具体场景
>
> ##### 场景 1：同步 I/O 操作
...
>   // 用户线程A执行阻塞式读取
>   char buf[1024];
>   read(fd, buf, sizeof(buf));  // 触发系统调用，内核线程阻塞
>   ```
>
> - 后果：
>
>   - 内核线程因等待磁盘 I/O 被挂起。
>   - 即使其他用户线程（如线程B）无需 I/O 且处于就绪态，也无法运行，因为内核线程已被阻塞。
>
> ##### 场景 2：锁竞争
>
> - 示例：用户线程尝试获取已被占用的互斥锁。
>
>   ```c
>   pthread_mutex_lock(&mutex);  // 若锁被其他用户线程持有，当前线程阻塞
>   ```
>
> - 后果：锁的持有者若因某种原因（如死锁）未释放锁，整个进程的所有用户线程都会被阻塞。
>
> ##### 场景 3：睡眠操作
>
> - 示例：用户线程调用 
...
> | **因素**               | **多对一模型的影响**                                         |
> | ---------------------- | ------------------------------------------------------------ |
> | **单内核线程承载**     | 所有用户线程的阻塞操作集中到一个内核线程，形成单点故障。     |
> | **内核无用户线程感知** | 操作系统无法主动调度用户线程，必须依赖用户态线程库的协作（但库无法绕过内核阻塞）。 |
> | **缺乏抢占式调度**     | 若一个用户线程长时间占用 CPU（如死循环），其他用户线程可能饿死（除非主动让出 CPU）。 |
...
> 线程之间的通信**不一定**需要系统调用（syscall），因为线程共享同一个进程的地址空间，可以直接通过共享内存（如全局变量、堆内存等）进行高效通信，无需内核介入。
>
> #### 线程通信的常见方式：
>
> 1. **共享内存（无需系统调用）**
>
>    - 线程可以直接读写进程内的全局变量、静态变量或动态分配的内存（如 `malloc` 的堆内存）。
>
> 2. **互斥锁（Mutex）、条件变量（Condition Variable）等同步机制**
>
>    - 虽然 `pthread_mutex_lock()`、`pthread_cond_wait()` 等函数在 POSIX 线程库中最终可能涉及系统调用（如 `futex`），但现代实现会在用户态优化，仅在竞争时才进入内核。
>
>      ```c
>      pthread_mutex_t lock;
>      pthread_mutex_lock(&lock);  // 可能触发系统调用（竞争时）
>      // 临界区操作
...
> 3. **信号量（Semaphore）**
>
>    - 类似互斥锁，`sem_wait()` 和 `sem_post()` 可能在某些情况下触发系统调用。
>
> 4. **原子操作（无系统调用）**
>
>    - 使用 `std::atomic`（C++）或 `__atomic_*`（GCC）等原子操作，依赖 CPU 指令（如 `x86` 的 `LOCK` 前缀），完全在用户态执行。
>
>      ```cpp
>      std::atomic<int> counter(0);
>      counter.fetch_add(1);  // 无系统调用
>      ```
>
> 5. **线程间信号（Signal）**
>
>    - 使用 `pthread_kill()` 发送信号，需要系统调用。
>
> #### 何时需要系统调用？
>
> - **竞争发生时**：如多个线程争抢互斥锁，可能触发 `futex` 系统调用让内核调度。
> - **显式同步**：如 `sem_wait()` 在资源不可用时可能阻塞线程，进入内核等待。
> - **跨进程线程通信**：如果线程属于不同进程（如 Linux 的 `CLONE_THREAD` 不同进程共享线程），则需要 IPC 机制（如共享内存 + 信号量），此时涉及系统调用。
>
> #### 总结
>
> - **无竞争时**：线程通信可以完全在用户态完成（如原子操作、无锁结构）。
> - **需要同步时**：可能间接使用系统调用（如 `futex`），但并非每次通信都触发。
> - **关键区别**：线程通信比进程通信更轻量，因为共享地址空间，减少了内核介入的需求。
...
- 操作系统只看到 **进程而不是用户级线程，所以它不能直接调度用户级线程**，也不能在**多处理器系统上并行运行用户级线程**；（但内核级线程可以）
- 用户级线程是由**用户程序或函数库实现的**，不依赖于操作系统支持。所以用户级线程可以在任何操作系统中运行。
...
- **由用户空间的线程库管理**（如 POSIX Pthreads），**不涉及内核**。
- 切换过程：
  1. **仅保存/恢复用户态寄存器**（如程序计数器 PC、栈指针 SP、通用寄存器）。
  2. **不涉及特权级切换**（用户态 → 内核态 → 用户态）。
  3. **不触发系统调用**，无需内核干预。
- 优点：
  - **极快**（通常只需几条指令）。
  - **不涉及内核调度**，完全由用户代码控制。
- 缺点：
  - **无法利用多核 CPU**（因为内核只看到 1 个 KLT）。
  - **一个 ULT 阻塞会导致整个进程阻塞**（如 I/O 阻塞）。

##### 内核级线程（KLT）切换

- **由内核管理**，每个 KLT 是独立的调度单位。
- 切换过程：
  1. **触发系统调用**（如 `sched_yield()` 或时间片耗尽）。
  2. **CPU 从用户态切换到内核态**（特权级切换）。
  3. **保存当前线程的完整上下文**（用户态 + 内核态寄存器、页表、FPU 状态等）。
  4. **选择新线程**（内核调度器决策）。
  5. **恢复新线程的上下文**。
  6. **CPU 从内核态返回用户态**。
- 优点：
  ...
  - **一个 KLT 阻塞不会影响其他 KLT**。
- 缺点：
  - **开销大**（涉及特权级切换、系统调用、完整上下文保存/恢复）。

- 同一进程的 ULT 共享地址空间，切换时：
  - **无需切换页表（TLB 无需刷新）**。
  - **无需切换进程 ID（PID）或其他内核资源**。
  - **仅需切换线程的栈和寄存器**。
- 对比跨进程切换：
  - 跨进程切换需要**切换页表、刷新 TLB、切换 PID**，开销更大。

- 同一进程的 KLT 仍然比跨进程切换快，因为：
  - **共享地址空间**（无需切换页表）。
  - **共享文件描述符、信号处理等**（无需重新加载）。
  - **但仍需内核介入**，比 ULT 切换慢。
