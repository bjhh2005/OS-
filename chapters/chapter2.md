...
> PC 值由中断隐指令（硬件）自动保存，而**通用寄存器**的内容由操作系统保存，块表和 Cache 内容由硬件机构保存，CPU 通过执行软中断指令将 CPU 的运行状态从用户态切换到内核态，这是由硬件完成的。
