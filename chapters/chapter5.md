...
- 进程通信机制：**共享内存、消息传递、管道、共享文件**

|   **特性**   |                 **消息缓冲通信**                  |             **信箱通信**             |
| :----------: | :-----------------------------------------------: | :----------------------------------: |
| **数据结构** |              消息队列（链表或数组）               |      固定大小的信箱（格子结构）      |
| **存储方式** |       动态分配缓冲区（如 `struct message`）       |         预分配固定大小的格子         |
| **消息组织** |               通常按 FIFO 顺序处理                | 可以支持优先级或分类（如按消息类型） |
| **典型实现** | UNIX/Linux 消息队列（`msgget`/`msgsnd`/`msgrcv`） | 更抽象，需用户自行实现（如邮箱系统） |
| **同步方式** | 通过信号量（`mutex`、`sm`）控制队列访问                  | 通过信箱的格子状态（空/满）控制         |
| **阻塞行为** | 支持阻塞/非阻塞发送和接收（如 `msgsnd` 的 `IPC_NOWAIT`） | 明确区分阻塞发送/接收和非阻塞发送/接收  |
| **死锁风险** | 可能因队列满/空导致阻塞                                  | 需避免“阻塞发送 + 非阻塞接收”的矛盾组合 |

> 两个合作进程无法利用**高级语言程序设计中的全局变量**来交换数据，因为全局变量是对于同一进程而言的，不同进程中就是不同的变量，无法用于交换数据。

|       **特性**       |         **共享文件**         | **共享存储区（Shared Memory）** |
| :------------------: | :--------------------------: | :-----------------------------: |
|       **本质**       | 通过文件系统（磁盘）共享数据 |     直接映射同一块物理内存      |
|   **数据存储位置**   |           磁盘文件           |         物理内存（RAM）         |
| **是否需要显式读写** |   是（`read()`/`write()`）   |     否（直接访问内存地址）      |
|   **是否需要同步**   |    是（文件锁、信号量等）    |     是（信号量、互斥锁等）      |
...
- 对于一个接收进程来说，在执行接收原语后也有两种可能选择：
  ...
  - 如果没有正在等待的消息，则该进程阻塞直到有消息到达；或者该进程继续执行，放弃接收的努力。前者称为**阻塞接收**，后者称为**非阻塞接收**。

- 根据发送进程和接收进程采取方式的不同，通常有三种常用的组合方式：

  - 非阻塞发送、阻塞接收。（生产者-消费者模型）
  - 非阻塞发送、非阻塞接收。（异步通信）
  - 阻塞发送，阻塞接收。（完全同步）

- **逻辑矛盾：发送者等待，接收者不等待**
  - 如果发送者阻塞等待接收者取消息，但接收者采用非阻塞方式（不等待），那么发送者可能会**永远阻塞**（如果接收者不主动取消息）。
  - 这种组合会导致**死锁风险**，因为发送者依赖接收者的行为，但接收者并不保证会处理消息。
