...
设一共$n$个资源，$m$个进程，每个进程最多请求$k$个资源，这样只要$n\ge(k-1)m+1$，即可确保不会发生死锁。
...
1. 判断系统是否是安全状态
   ...
   1. 求出最大资源量`MAX`，分配量`Allocation`，需求量`Need`，可供使用量`Available`.
   2. 列表格：可供使用量`Work`，分配量`Allocation`，需求量`Need`，回收后可供使用量`Work + Allocation`，是否可满足`T/F`
   3. 每项更新时，`Work`的初始值为最初计算的`Available`，而后不断判断`Available`可以满足哪个进程的`Need`，如果没有的话则系统不安全；如果有的话，则把他列如表格，计算各项，在计算下一项时，把`Work`更新为上一行计算的`Work + Allocation`.
   4. 如果最终所有进程都可以满足分配，则证明当前系统安全；

   > 当系统处于安全状态时，系统中一定无死锁进程；当系统处于不安全状态时，系统中不一定会出现死锁进程。

2. 一个进程又提出了一个资源分配请求，系统能否去满足他的请求

   1. 判断请求`Request`是否满足`Request`$\le\min\{$`Need`,`Available`$\}$.如果不满足，则直接拒绝。如果可以则更新`Available`,`Need`和`Allocation`.
   2. 再次判断当前系统是否安全，如果安全则满足，如果不安全则拒绝。
