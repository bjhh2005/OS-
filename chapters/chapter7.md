...
- **主存和寄存器** 是 CPU **唯一可以直接访问** 的存储设备。**内存单元只看到一个流**：地址+ 读请求，或地址、数据和写请求。
  ...
  > 对主存的**访问**是以**字节**为单位的，对主存的**分配**是以**页**为单位的。
...
> 在计算机系统中，**虚拟地址空间（Virtual Address Space）** 是操作系统为每个进程提供的 **逻辑地址范围**，**与物理内存或硬盘无关**。
>
> ### 1. 虚拟地址空间的本质
>
> - **逻辑视角**： 
...
- **类型**
  ...
  - **动态链接（运行时）**：在程序运行时由操作系统加载共享库（如 `.dll` 或 `.so`）。
    ...
    - 这个要求使用**段式内存管理**或者**段页式内存管理**便于进行扩展与共享。
    ...
    - |       **特性**       |       **装入时动态链接（Load-time Linking）**       |    **运行时动态链接（Run-time Linking）**    |
      | :------------------: | :-------------------------------------------------: | :------------------------------------------: |
      |     **触发时机**     | 程序启动时，由操作系统的动态链接器（如`ld.so`）完成 | 程序运行中，通过API（如`dlopen()`）手动触发  |
      |     **内存占用**     |    所有依赖库在启动时加载，可能包含未使用的代码     |       仅加载实际需要的库，按需分配内存       |
      |     **典型场景**     |      常规应用程序（如`gcc -lxxx`链接的标准库）      |  插件系统、热更新、延迟加载（如游戏资源包）  |
      | **与页式存储的协作** |               不直接利用按需分页机制                |   完美匹配按需分页，库代码按需加载到物理页   |
      |     **错误处理**     |          启动时检查所有库，缺失则直接报错           | 可动态处理库缺失（如降级功能或提示用户安装） |

|   **阶段**    |     **地址类型**     |  **负责方**  |             **示例**             |
| :-----------: | :------------------: | :----------: | :------------------------------: |
|   **编译**    | 段内偏移（逻辑地址） |    编译器    | `.text`段内`bar`函数偏移`0x100`  |
| **静态链接**  |       虚拟地址       |    链接器    | `bar`地址 = `0x08048000 + 0x100` |
| **动态链接**  | 虚拟地址（延迟绑定） |  动态链接器  |    `printf`地址运行时填入GOT     |
| **加载/运行** |       物理地址       | 操作系统+MMU | 页表将`0x08048100`映射到物理页框 |

> **程序的装入**是指将可执行程序从磁盘加载到内存中，使其能够被CPU执行的过程。
>
> |           **场景**           | **装入时动态链接** | **运行时动态链接** |
> | :--------------------------: | :----------------: | :----------------: |
> |  核心库（如`libc`, `libm`）  |        必须        |       不可靠       |
> |    全局变量/静态符号绑定     |        支持        |       不支持       |
> | 构造函数（`main()`前初始化） |      自动执行      |     需手动触发     |
> |    高频调用库（性能敏感）    |       零开销       |     有解析延迟     |
> |   强制依赖（安全关键程序）   |      严格检查      |     可能漏加载     |

#### 虚拟地址转换为物理地址

虚拟地址指的是**程序访问内存时使用的地址**，操作系统负责将其转换为物理地址。
...
> 当 CPU 形成一个有效地址时，查找页表的工作是由**硬件**实现的，不需要操作系统的参与。
...
- **内存占用过高**：
  ...
  若每个页表项（PTE）占 4 字节，单级页表需 $2^{20}\times4B=4MB$内存。
  在 64 位系统中，单级页表的大小完全不可行（如 48 位地址空间需要 $2^{36}$ 个页表项）。
- **访问速度慢**：
  每次地址转换需访问 **内存中的页表**，导致延迟（地址转换的核心过程是在 **CPU 中的内存管理单元（MMU）** 中完成的）。
...
|         **技术**         |    **解决的问题**    |            **优势**            |             **代价**             |
| :----------------------: | :------------------: | :----------------------------: | :------------------------------: |
| **多级页表**（后续展开） | 单级页表内存占用过大 |       按需分配，节省内存       |    查表次数增加，未命中时更慢    |
...
- **执行时**
  ...
  - **物理地址延迟绑定**：程序使用虚拟地址，实际物理地址在指令访存执行时由 MMU 转换（如分页机制）。
  - **代码不可修改**：程序代码段被标记为只读，防止执行时被篡改，重定位信息由操作系统通过页表管理。

动态重定位（Dynamic Relocation）是现代操作系统内存管理的核心技术之一，它允许程序在**运行时**被移动（如内存紧凑、交换、按需分页等），而无需重新编译或加载。
...
> - 机制：
>   - 移动程序时：
>     - 操作系统修改基址寄存器（如从 `0x1000` 改为 `0x3000`）。
>     - 后续指令访存时，MMU 自动计算新物理地址，无需程序感知。
>
> #### (2) 分页机制（现代通用方案）
>
> - **适用场景**：非连续内存分配（虚拟内存系统）。
> - 机制：
>   - 移动程序时：
>     - 操作系统只需修改页表项（如将虚拟页 `2` 从物理页帧 `5` 改为 `8`）。
>     - CPU 通过 MMU 查询页表，自动转换地址，程序无感知。
> - 优势：
>   - 支持**部分移动**（如仅移动某些页，无需移动整个进程）。
>   - 结合缺页中断，实现按需加载和页面交换。

#### 动态加载与链接

程序是由主程序和许多子程序组成，整个程序不需要完全存储到内存中才能执行，**只有在调用子程序时才会加载该子程序**，这样可以更好地利用内存空间。因为未使用的子程序不会被加载。
...
- 操作系统检查例程是否在进程的内存地址中，如果不在地址空间中，则添加到地址空间中
...
1. 重定位寄存器（Relocation Register）
   - 存储进程的 **物理内存基址**（起始地址）。
   - 系统中只有1个重定位寄存器，每次切换进程时，都要保存和恢复该寄存器的值。
...
3. 地址转换（Address Translation）：
   ...
   - 示例：逻辑地址 `0x300` + 基址 `0x1000` → 物理地址 `0x1300`。
4. 访问物理内存：
...
- 若紧凑时进行了 I/O 操作，数据将到达错误的位置
  ...
  > #### (1) 紧凑会改变进程的物理地址
  >
  > - 紧凑时，操作系统会**移动进程的内存块**（调整基址寄存器）。
  > - 如果进程正在执行 I/O 操作（如磁盘读写），其数据缓冲区（Buffer）的物理地址可能被移动，导致：
  >   - **数据写入错误位置**（如磁盘数据写入旧地址，但进程已移动）。
  >   - **数据读取错误**（如从旧地址读取，但数据已被覆盖）。
  >
  > #### (2) 示例场景
  >
  > - **进程 A** 正在从磁盘读取数据到内存地址 `0x1000`。
  > - 操作系统执行紧凑，将进程 A 的内存块从 `0x1000` 移动到 `0x2000`。
  > - 问题：
  >   - 磁盘控制器仍然向`0x1000`写入数据，但进程 A 的缓冲区已移动，导致：
  >     - **数据丢失**（写入到错误的空闲内存）。
  >     - **数据损坏**（可能覆盖其他进程的数据）。
  >
  > ### 2. 解决方案：锁定进程 + 使用 OS 缓冲区
  >
  > #### (1) 锁定参与 I/O 的进程
  >
  > - 禁止紧凑移动正在执行 I/O 的进程：
  >   - 在 I/O 操作期间，该进程的内存块**保持固定**（不参与紧凑）。
  >   - 确保 I/O 操作始终访问正确的物理地址。
  >
  > #### (2) 使用操作系统缓冲区（Double Buffering）
  >
  > - I/O 数据不直接写入进程内存，而是先写入 OS 缓冲区：
  >   1. 磁盘数据先被读取到**内核缓冲区**（OS 管理的临时内存）。
  >   2. 紧凑完成后，数据从 OS 缓冲区**安全复制**到进程的最终内存位置。
  > - 优点：
  >   - 紧凑可以自由移动进程内存，不影响 I/O 操作。
  >   - 数据不会因地址变化而丢失或损坏。
...
前面提到的都是**基于顺序搜索的分配算法**，基于索引搜索的分配算法有：快速适应算法、伙伴系统、哈希算法等。

##### 快速适应算法

1. 基本思想

   - 维护多个空闲分区链表，每个链表存储特定大小的空闲块
...
2. 实现方式
   ...
   - **空闲块合并**
     ...
     快速适应算法通常采用 **边界标记法（Boundary Tags）** 实现合并：

     - **每个内存块首尾** 存储标记（包含块大小、使用状态等信息）
     - 释放内存时：
       1. 检查 **前驱块** 是否空闲（通过当前块首部标记）
       2. 检查 **后继块** 是否空闲（通过当前块尾部标记）
       3. 若相邻块空闲，则合并为更大的块，并 **从原链表中移除**，插入到更大尺寸的链表
   ...
   - **分配后剩余块的调整**
     ...
     1. 若剩余块足够大（超过最小阈值）：
        - 将剩余块**从当前链表移除**
        - 根据其新大小，插入到 **对应尺寸的空闲链表**
     2. 若剩余块过小（小于最小阈值）：
        - 直接作为 **内部碎片** 保留，不单独管理（避免链表过多小碎片）
...
void deallocate(Block *block) {
    ...
    while (current_order < MAX_ORDER) {
        // 计算伙伴地址
        Block *buddy = (Block*)((size_t)block ^ (1 << current_order)); // 利用异或翻转第k位
...
- 若 $A$ 是逻辑地址，$L$ 是页面大小，则：
  ...
  - 页内位移：$W=A\%L$
- 静态重定位是在程序运行之前由装载程序完成的，必须分配其要求的全部**连续内存空间**（**连续内存**确保程序的所有指令和数据能够通过简单的基址偏移计算正确访问。若内存不连续，偏移计算会失效，导致访问错误），而页式存储管理方案是将程序程序离散分为若干页，从而将程序装入不连续的内存空间。
...
> C 程序经过预处理、编译、汇编、链接产生可执行文件，在链接的前一步产生可重定位的二进制目标文件。C 语言采用源文件独立编译的方法，例如将程序`main.c`, `file1.c`, `file2.c`, `file1.h`, `file2.h`在链接前一步生成`main.o`, `file1.o`, `file2.o`，这些目标模块的逻辑地址都从0开始，链接器将这3个文件、`libc`和其库文件链接成一个可执行文件，从而形成整个程序的完整逻辑地址。例如`file1.o`的逻辑地址可能会从`0-1023`变成`1024-2047`.
...
查找时间是 n ，命中率为 p，则有效存取时间（假定**忽略快表更新时间**）：
$$
EAT = p*(n+m)+(1-p)(2m+n)
$$
...
> - 在多级页表中，页表基址寄存器存放的是**顶级页表的起始物理地址**，即一级页表的起始物理地址。
...
> - **设计权衡**：通过多级页表减少内存占用（仅需为活跃的地址区域分配页表），但代价是地址转换复杂度增加（需多次访存）。实际系统中，通过 **TLB（快表）** 和 **大页机制** 优化性能。
> - 而这种惰性加载方式单级页表是不支持的。因为单级页表本质上是一个线性数组，页表项通过公式计算：$PTE地址=页表基址+页号\times页表项大小$.
>
> 类似的，如果是48位逻辑地址空间，计算得出应该是4级页表。
...
现代操作系统一般允许**大逻辑地址空间**，这使得页表太大，为解决 **页表占用大量存储空间** 的问题，引入了反向页表。反向页表为每个物理块设置一个页表项，并将它们 **按物理块号大小排序**，表项内容为 **页号及其隶属进程的标识号**。
...
>   
>   操作系统没有能力检测到调用树，所以必须由用户来声明，增大了用户编程负担，现在已经不常见了。
>
...
- **进程的换出：** 先选择换出进程（阻塞、优先级低、驻留时间长），再申请对换空间，然后启动磁盘写，若成功则可释放其内存空间并修改数据结构，会写入到**对换区**（连续分配，在磁盘中，提高IO速度，只需要一次交换就行。而文件区为了提高存储空间利用率，采用的是离散分配方式）

  > 为了保持对换出进程的管理，操作系统会保留进程的PCB不换出，只会换出其他部分。
...
- **分段管理** 以用户需求为核心，通过逻辑段反映程序结构（如代码、数据、堆栈），更贴近程序设计思维，但易产生外部碎片。
  - **程序员控制**：通过编译器指令（如GCC的`section`属性）或汇编语言显式分段。
  - **编译器默认**：高级语言中，编译器自动按功能分段（代码/数据/堆栈等）。
  - **运行时补充**：操作系统管理段的内存分配，动态段（如堆、栈）在运行时调整。
...
- **虚拟内存 ：** 将用户**逻辑内存**与**物理内存分开**，虚拟存储器的理论基础是程序执行时的局部性原理。**局部性原理**是指程序在执行过程中一个较短时间内，程序所执行的指令地址和操作数地址分别局限于一定区域内。**虚拟存储器**是指具有请求调入和置换功能，能从逻辑上对内存容量加以扩充的一种存储器系统。只有部分运行的程序需要在内存中。虚拟存储器是一种 **以时间换空间** 的技术。
  ...
  - 允许若干个进程共享地址空间
  - 允许更多有效进程创建
  - 虚拟存储技术是基于页或段从内存的调入(调出)实现的，需要有**请求**机制的支持
...
3. **页面/段置换**
   - **内存不足时**：操作系统根据 **置换算法**（如 LRU、FIFO）选择内存中 **暂时不用的页面/段**，将其写回外存以腾出空间，再载入新内容。
...
> 虚拟存储器的最大容量由**计算机的地址结构**决定，与主存和外存的容量无关，对于32位系统来说，其虚拟地址空间为$2^{32}B$.

#### 请求分页存储管理 Demand paging

请求分页存储管理方法是在分页存储管理的基础上增加了 **请求调页**和**页面置换功能**。主要支持机构有：页表、缺页中断机构、地址变换机构、请求调页和页面置换软件。

影响请求分页系统有效（平均）访存时间的有：缺页率、磁盘读写时间、内存访问时间、执行缺页处理程序的 CPU时间。
...
|  **对比维度**  |                         **缺页中断**                         |                         **一般中断**                         |
| :------------: | :----------------------------------------------------------: | :----------------------------------------------------------: |
|  **触发原因**  | 由 CPU 执行指令时检测到 **页面未驻留内存**（页表项标记为无效）  | 由 **外部设备或定时器** 触发（如 I/O 完成、时钟中断、硬件故障等） |
|  **产生时机**  | 在 **指令执行过程中** 发生（如访问内存阶段），中断当前执行流程 | 在 **指令执行间隙**（CPU 检查中断标志的固定时间点），不打断单条指令执行 |
|  **中断类型**  |         属于 **异常（Exception）**，由 CPU 自身逻辑触发         |        属于 **硬件中断（Interrupt）**，由外部信号触发         |
|  **可重复性**  | 一条指令可能引发 **多次缺页中断**（如 `COPY A→B` 需访问 A 和 B 的多个未驻留页） | 一条指令执行期间最多响应 **一次中断**（中断响应后屏蔽后续信号直到处理完成） |
...
2. **页在主存且页表项不在快表中** 
   - 查快表时间：$\varepsilon$  
   - 查页表时间：$t$  
   - 修改快表时间：$\varepsilon$  （因为是并行的，所以不计算）
   - 访问内存时间：$t$  
   - **总计**：$\varepsilon + t + t =\varepsilon + 2t$  
   
3. **页不在主存（需处理缺页中断）** 
   - 查快表时间：$\varepsilon$  
   - 查页表时间：$t$  
   - 处理缺页中断时间：$t_1$  
   - 重新执行访存指令 $\varepsilon+t$
   - **总计**：$2(\varepsilon + t) + t_1$  

**符号说明**：  

- $\varepsilon$：查快表时间  
- $t$：访问内存时间（或查页表时间，假设两者耗时相同）  
- $t_1$：处理缺页中断的总时间（含 I/O 操作、页表更新、快表更新等）  

综合三种场景的概率加权，有效存取时间（EAT）为：
$$
EAT = \alpha(\varepsilon + t) + (1 - \alpha)\left [(1 - f)(2\varepsilon + t) + f(t_1 + 2\varepsilon + t)\right]
$$

- 第一项：快表命中的情况（$\alpha$ 概率）  

- 第二项：快表未命中时，分两种子情况：  
  - $(1-f)$ 概率页在主存（耗时 $2\varepsilon + t$）  
  - $f$ 概率缺页（耗时 $t_1 + 2\varepsilon + t$）
  
  > 注意这个只是一级页表的情况，对于多级页表内存访问的次数会增多
##### 写时拷贝（Copy-on-Write, COW)
写时拷贝允许父子进程在内存中共享页面，如果任一进程需要对页面进行写，那么就创建 **一个共享页的副本**，只有 **可能修改的页** 才需要标记为写时拷贝。COW 通过 **共享只读 + 写时复制** 的延迟策略，以 **缺页异常** 为触发器，在保证进程隔离性的同时，最大化内存利用率与性能。

|     **优势**     |                       **说明**                       |
| :--------------: | :--------------------------------------------------: |
| **内存高效利用** |       共享页只读时不复制，减少冗余内存占用）。       |
|   **性能优化**   | 避免无修改时的拷贝开销，降低进程创建和内存分配延迟。 |
|  **进程隔离性**  |      修改后的页面独立，确保进程间数据互不干扰。      |

1. 进程创建（`fork()`）：
   - 子进程与父进程共享所有内存页，仅在子进程或父进程修改时复制目标页。
2. 内存映射文件（mmap）：
   - 多个进程映射同一文件时，写操作触发 COW 生成私有副本。
3. 虚拟化与容器：
   - 虚拟机/容器镜像的快速克隆依赖 COW 技术减少存储冗余。

> ### Example
>
> ![写时拷贝技术示例](../img/写时拷贝技术示例.png)
>
> 1. **初始状态**：
>    - **进程 1** 与 **进程 2** 共享物理内存中的页面 A、B、C（图中 `pageA`、`pageB`、`pageC`）。
>    - 所有共享页面被标记为 **只读**（Read-Only），但进程逻辑上认为这些页面可读写（操作系统通过页表权限位实现此机制）。
> 2. **触发修改**：
>    - **进程 1** 尝试修改 **页面 C**（如写入数据）。
>    - 由于页面 C 的页表项标记为只读，触发 **缺页异常/中断**（Page Fault）。
> 3. **内核介入**：
>    - 操作系统捕获该异常，检测到此次访问是 **写时拷贝场景**（进程试图修改共享的只读页）。
>    - **内核为进程 1 分配新的物理页面**（图中 `Copy of pageC`），并将原页面 C 的内容复制到新页面。
> 4. **更新映射**：
>    - 修改 **进程 1 的页表项**，使其指向新复制的页面，并将权限设置为 **可写**。
>    - **进程 2 的页表项保持不变**，仍指向原页面 C（维持共享状态）。
> 5. **最终结果**：
>    - **进程 1** 使用独立的 `Copy of pageC` 进行修改，进程 2 继续共享原 `pageC`。
>    - 物理内存中同时存在原页面 C 和新副本，但 **仅在被修改时产生实际复制开销**。


#### 请求分段存储管理 Demand segmentation

**核心思想**：按需将程序的 **逻辑段**（如代码段、数据段、堆栈段）动态调入内存，而非一次性全部加载。

1. **段表管理：**
   - 每个进程维护一个 **段表**，记录各段的基址、长度、访问权限及驻留位（是否在内存）。
2. **访问流程：**
   - 执行指令时，根据逻辑地址的段号查询段表：
     - 若段已驻留内存 → 直接访问物理地址。
     - 若段未驻留 → 触发 **缺段中断**，由操作系统从外存调入该段，更新段表并分配连续内存空间。
3. **内存分配：**
   - 采用动态分区分配（如首次适应算法），易产生 **外部碎片**，需定期内存紧缩。

#### 请求调页的段页式存储管理

**分段与分页结合**，先按逻辑分段，再将每段划分为固定大小的页，按需调页。

1. **地址结构：**
   - 逻辑地址 = **段号 + 段内页号 + 页内偏移**（三维地址）。
2. **数据结构：**
   - **段表**：记录各段的页表基址和段属性（如权限）。
   - **页表**：每段对应一个页表，记录页框号及驻留位。
3. **访问流程：**
   - 根据段号查段表 → 根据段内页号查页表 → 若页未驻留 → 触发 **缺页中断**，调入页并更新页表。
4. **内存分配：**
   - 以页为单位分配物理内存，避免外部碎片（但存在内部碎片）。
...
注意**缺页次数**和**换页次数**是不一样的，最开始初始化的时候，只缺页但不换页。

#### FIFO

选择 **调入主存时间最长的页面** 予以淘汰。

实现比较简单，对按线性顺序访问的程序比较合适，但可能产生异常现象。很少使用纯粹的 FIFO 。

**Belady 异常：** 在某些情况下，分配给进程的页面数增多，缺页次数反而增加。

*若图书馆仅根据书籍入库时间决定下架（类似 FIFO），而非借阅频率（类似 LRU），即使扩大书架容量，也可能因保留大量无人问津的旧书，导致热门新书频繁下架再上架，整体借阅效率反而下降。*

#### 最佳置换算法(OPTimal replacement, OPT)

也称 **最优置换算法**。从内存中选择**最长时间不会使用**的页面予以淘汰。

特点：因页面访问的未来顺序很难精确预测，该算法具有理论意义，可以用来 **评价其他算法的优劣**。如与最佳相比最坏不差 12.3%，平均不差 4.7%，就是可用算法。

#### 最近最久未使用置换算法(Least Recently Used, LRU)

选择**最近一段时间内最长时间没有被访问过的**页面予以淘汰。为此，应赋予每个页面一个访问字段，用于记录页面**自上次访问**以来所经历的时间。LRU 算法实现时需要 **较多的硬件支持**，以记录进程中各页面自上次访问以来有多长时间未被访问。下面介绍几种实现方法。

导致 LRU 算法实现起来耗费高的原因是：**需要对所有的页进行排序**

- **计数器法：** 为每个页面配置一个计数器 其初值为 0 。当进程访问某页时，将计数器的最高位置 1，定时器每隔一定时间将计数器右移 1 位，则 **数值最小** 的页是最近最久未使用的页面。
- **特殊栈：** 利用一个特殊栈保存当前使用各页的页面号。当进程访问一个页面时，将该页面号从栈中移出压到栈顶。栈底即最近最久未使用的页面号。

最佳置换和 LRU 都 **没有 belady 异常**，这类算法称为 **栈算法**。对于栈算法：帧数为 $n$ 的内存页集合是帧数为 $n+1$ 的页集合的子集。即，增加一帧后，原内存中的所有页面仍会被保留，新帧仅用于容纳额外的页面，**不会淘汰原有页面**。

> 这一性质的本质是 **栈算法的无后效性**，其依赖以下两个关键特性：
>
> 1. **未来无关性**：
>    栈算法的置换决策**仅依赖当前的页面访问历史**​（如 LRU 基于最近访问时间），与未来无关。
> 2. **单调包含性**：
>    当内存资源（页帧）增加时，算法**不会因容量扩大而改变原有页面的保留状态**，新资源仅用于扩展缓存能力。
>
> #### **为什么 Belady 异常不会出现在栈算法中？**
>
> - **Belady 异常的根源**：FIFO 等非栈算法因 **忽略访问历史**，增加页帧可能导致淘汰关键页面，破坏局部性。
> - 栈算法的免疫性：
>   - 栈算法始终保留历史活跃页面（如 LRU 保留最近使用的页），新增帧仅用于缓存更多页面，**不会触发非理性淘汰**。
>   - 因此，**缺页率随帧数增加单调递减**，符合直觉。
> 
>这一性质揭示了栈算法的**资源扩展友好性**：内存扩容时，原有缓存内容不会被破坏，仅需增量管理新资源。这种“只增不减”的特性，使其成为高可靠性系统的理想选择。

#### LRU 近似置换算法（Least Recently Used  Approximation Algorithm, LRUAA）

每个页表项都关联一个引用位，初始值为 0

- 当页被访问时将引用位设为 1

- 如果存在引用位为 0 的页，则置换它

**二次机会算法思想：**使用 FIFO 算法选择一页淘汰时，先检查该页的引用位，如果是 0 就立即淘汰该页，如果是 1 就给它第二次机会，将其引用位清 0 ，并将它放入页面链的末尾，将其装入时间置为当前时间，然后选择下一个页面。

##### 简单时钟算法（最近未使用算法 Not Recently Used, NRU）

也叫 CLOCK 算法。

该算法将内存中所有页面都通过链接指针链接成一个**循环队列**，设置访问位，1表示被访问过，0表示未被访问过；如果访问位是0，则选择该页换出，如果访问位是1，则将访问位置0，暂不换出，再按照**FIFO算法**检查下一个页面，重复上述步骤。

![普通的 CLOCK 算法](../img/普通的 CLOCK 算法.png)

- 某一页装入主存时，要将**访问位**置为1，如果该页之后又被访问到，访问位也还是标记为1
- 只有发生缺页中断并需要置换页的时候，指针才会移动

将一个修改过的页面换出需要写磁盘，其开销大于未修改页面。为此在改进型时钟算法中**应考虑页面修改情况**。设 R 为访问位， U为修改位，将页面分为以下 4 种类型：

- 1 类 (R=0,U=0) 未被访问又未被修改
- 2 类 (R=0,U=1) 未被访问但已被修改
- 3 类 (R=1,U=0) 已被访问但未被修改
- 4 类 (R=1,U=1) 已被访问且已被修改

1. 从指针当前位置开始扫描循环队列，寻找R=0,U=0 的页面，将满足条件的第一个页面作为淘汰页。
2. 若第 1 步失败，则开始第 2 轮扫描，寻找R=0,U=1 的页面，将满足条件的第一个页面作为淘汰页，并将所有经历过页面的访问位置 0 。(之所以优先R=0而不是U=0，是因为处理缺页中断的时间开销要高于处理IO的时间开销)
3. 若第 2 步失败，则将指针返回到开始位置，然后重复第 1 步，若仍失败则必须重复第 2 步。此时一定能找到淘汰页面。
4. **特点：减少了磁盘 I/O 次数，但算法本身开销增加。**

![改进的 CLOCK 算法](../img/改进的 CLOCK 算法.png)

##### 基于计数的页面置换（最不经常使用算法，Least Frequently Used, LRU）

选择到当前时间为止访问次数最少的页淘汰。该算法要求为每页设置一个访问计数器，每当页被访问时，该页的访问计数器加 1 。发生缺页中断时，淘汰计数值最小的页面，并将所有计数器清零。

选择出的页面**不是立即换出**，而是**放入两个链表之一** 。

##### 页面缓冲算法

算法采用 FIFO 选择淘汰页，规定将淘汰页放入两个链表之一：

- **空闲链：**页面**未修改**则放入空闲链末尾

- **修改链：**页面**已修改**则放入修改链末尾

当需要访问一个页面时，若该页在上述两个队列中，则只要将该页面从队列中移出 ，否则用空闲链的第一个物理块装入该页。当修改链中的页面数达到一定值时，再将它们**一次写回磁盘**，然后将它们加入**空闲页面链表**中。

> ### 例子详解：基于 FIFO 的双链表页面置换算法
>
> 这个算法是 **FIFO（先进先出）的改进版本**，通过将淘汰页按**是否被修改**分为两个链表（空闲链和修改链），并采用**批量写回磁盘**的策略，减少 I/O 开销。下面通过一个具体例子说明其运行机制。
>
> ### 1. 初始状态
>
> 假设：
>
> - **物理内存有 3 个页框**（初始为空）。
> - **页面访问序列**：`A(读) → B(写) → C(读) → D(写) → A(读) → E(写)`
>    （括号内表示访问类型，"读"表示未修改，"写"表示已修改）。
> - **空闲链（Free List）**：`空`
> - **修改链（Modified List）**：`空`
>
> ### 2. 逐步执行过程
...
>
> - **A 在空闲链中**（未被淘汰到磁盘）。
> - **直接从空闲链移出 A**，替换 C（FIFO 选择最早进入的 B，但 B 被修改，不能直接淘汰）。
> - **C 未被修改**，放入**空闲链末尾**。
> - **内存状态**：`[D, B, A]`
> - **空闲链**：`[C]`
> - **修改链**：`空`
...
> ### 3. 批量写回磁盘
>
> 假设系统规定：**当修改链长度 ≥ 2 时，触发批量写回**。
>
> - 当前 **修改链**：`[D]`（未达到阈值，不写回）。
>
> - 如果后续再有修改页加入（如 F(写) 导致 B 被淘汰到修改链），则：
>
>   - **修改链**：`[D, B]`（达到阈值）。
>
>   - 系统将 D 和 B 一起写回磁盘，然后清空修改链，并将它们加入
>
>     空闲链：
>
>     - **空闲链**：`[C, D, B]`
...
> | **机制**               | **作用**                         | **优势**                |
> | ---------------------- | -------------------------------- | ----------------------- |
> | **空闲链（未修改页）** | 存放未被修改的淘汰页，可快速重用 | 减少不必要的磁盘写回    |
> | **修改链（脏页）**     | 暂存被修改的淘汰页，**批量写回** | 降低 I/O 次数，提升性能 |
> | **FIFO 置换策略**      | 选择最早进入的页淘汰             | 实现简单，适合顺序访问  |
>
> ### 5. 对比传统 FIFO
>
> - **传统 FIFO**：每次淘汰都要判断是否写回，导致**频繁单次 I/O**。
> - 本算法：
>   - 未修改页放入空闲链（可立即重用）。
>   - 脏页积累到一定数量后**批量写回**，减少磁盘操作。
>
> **适用场景**：
>
> - 适合**写操作较多**的环境（如数据库缓存）。
> - 不适合**突发大量写**的情况（可能堆积过多脏页，增加延迟）。
>
> 这个算法通过 **双链表（空闲链 + 修改链） + 批量写回**，优化了传统 FIFO 的 I/O 效率，是 **FIFO 的实用改进版本**。
...
- **单地址**指令且采用**直接寻址**，则所需最少物理块数为 2 。
- 若**单地址**指令且允许采用**间接寻址**，则所需最少物理块数为 3 。
- 某些功能较强的机器，指令及源地址和目标地址均跨两个页面，则最少需要 6 个物理块。

| **寻址方式** | **最少物理块数** |           **关键原因**           |
| :----------: | :--------------: | :------------------------------: |
|   直接寻址   |        2         |    指令与操作数可能分属不同页    |
|   间接寻址   |        3         |    指令、指针、操作数需独立页    |
| 复杂跨页指令 |        6         | 指令、源数据、目标数据均可能跨页 |
...
- **按优先级分配** Priority allocation，有两种方案
  - 将可供分配的物理块分为两部分，一部分**按比例分配**给每个进程，另一部分**根据进程的优先级**适当**增加物理块**。
  - 先**按比例为进程分配**物理块，当高优先级进程发生缺页时，允许它从低优先级进程处获得物理块。

页面分配常采用**固定分配**和**可变分配**两种策略。页面置换也有**全局置换**和**局部置换**两种策略：固定分配局部置换、可变分配全局置换、可变分配局部置换。

在请求分页管理中分配给进程的物理块的集合称为**驻留集**
...
> 采用页式虚拟存储管理和固定分配局部置换策略的系统中，数组采用行优先存储，页框大小为512B，有如下代码段：
>
> ```c
> int a[128][128];
> for(int i = 0; i < 128; i++)
...
> ```
>
> 系统为该进程只分配了一个页框，则执行过程中会发生$128\times128=16384$次中断。
>
> 系统每次访问到`a[i][0]`的时候，都会把一整行的数据调入内存，每次都会触发一次缺页中断，所以一共16384次。

##### 可变分配+全局置换

先为系统中的每个进程分配一定数量的物理块，操作系统本身也保持一个**空闲物理块队列**。当某进程发生缺页时，由系统从空闲物理块队列中取出一个物理块分配给该进程；当空闲物理块队列中的物理块用完时，操作系统才从内存中选择一页调出，该页可以是系统中**任何一个进程**的页面。

##### 可变分配+局部置换

根据某种原则为每个进程分配一定数量的物理块，当进程发生缺页中断时，只允许从**该进程的页面中选出一页换出**。如果某进程的缺页率**较高**，则系统**再**为该进程**分配**若干物理块；反之若某进程的缺页率**特别低**，则系统**可适当减少分配**给该进程的物理块数。

### 抖动

如果运行进程的**大部分时间都用于页面的换入，换出**，而几乎不能完成任何有效的工作，则称此进程处于**抖动状态**。抖动又称为**颠簸、颤动**。

此时应该增大内存容量，而不是增大物理交换区的容量。交换区只是临时存储被换出的页面，**增大交换区并不能减少缺页次数**，只是让系统能存放更多“冷”页面（不活跃的页面）。
...
![CPU利用率与程序道数的关系](../img/CPU利用率与程序道数的关系.jpeg)

**程序道数过多时（高并发）**

- **内存不足** → 频繁换页（缺页中断），CPU时间浪费在管理内存上。
- **锁竞争激烈** → 进程互相阻塞，CPU空转。
- **调度开销大** → 进程切换（Context Switch）消耗大量CPU时间。
- **最终结果**：CPU真正用于计算的时间减少，利用率**不升反降**。

进程分配的物理块太少、置换算法选择不当、全局置换使抖动传播

#### 工具集模型

**工具集模型**（Toolbox Model）是一种用于描述和优化计算机系统资源管理的理论框架，尤其在**操作系统调度、内存管理和并发控制**中广泛应用。其核心思想是：

> 将系统资源（CPU、内存、I/O等）视为“工具箱”中的工具，程序（进程/线程）是“工人”，如何高效分配工具决定了整体效率。

|     **组件**     |   **对应现实**    |           **计算机系统示例**            |
| :--------------: | :---------------: | :-------------------------------------: |
|    **工具箱**    |      资源池       |    CPU核心、内存、磁盘带宽、网络带宽    |
|     **工具**     |     单个资源      | 一个CPU核心、1GB内存块、一个磁盘I/O通道 |
|     **工人**     |     进程/线程     |          运行的应用程序或服务           |
|     **任务**     | 程序的计算/IO需求 |      矩阵计算、文件读取、网络请求       |
| **工具分配策略** |     调度算法      |  时间片轮转、优先级调度、内存置换算法   |

**工作集**是指某段时间间隔内，进程要访问的页面集合。一般来说，工作集$W$由时间$t$和工作窗口大小$\Delta$来确定。只需要数从$t$向前的$\Delta$内的页面集合，记得去重即可。

例如$1,4,2,3,5,3,2,t_1$;则在$t_1$前，取$\Delta=5$，$W=\{2,3,5\}$

工作集不一定是驻留集的子集，因为有些工作集中的页面可能还未被调入内存，或已被换出内存。只有当工作集完全包含在驻留集中时，才能保证进程不发生缺页中断。

#### 抖动的发现

##### 全局范围技术

全局范围技术采用时钟置换算法，用一个计数器 C 记录搜索指针扫描**页面缓冲**的速度。

- 若 C 的值**大于**给定的上限值，说明**缺页率太高（可能抖动）**或找不到可供置换的页面，这时应**减少程序道数**。
- 若 C 小于给定的下限值，表明缺页率小或存在较多可供置换的页面，这时应增加程序的道数。

###### 时钟置换算法的工作流程

1. 扫描页面链表：
   - 检查当前指针指向页面的访问位（Reference Bit）。
     - **访问位=1**：页面近期被访问过，给予“第二次机会” → 将访问位置0，指针后移。
     - **访问位=0**：选中该页面置换。
2. 统计计数器 C：
   - 记录本次扫描中指针移动的总次数（即检查的页面数），直到找到置换目标。

###### C 值与缺页率的关联

- 高缺页率场景：
  - 大多数页面的访问位=1（频繁使用） → 需多次扫描才能找到访问位=0的页 → **C 值增大**。
  - 例如：扫描了20页才找到可置换页 → C=20。
- 低缺页率场景：
  - 许多页面的访问位=0（冷页） → 快速找到置换目标 → **C 值减小**。
  - 例如：扫描3页即找到可置换页 → C=3。

##### L=S准则

实际证明，**产生缺页的平均时间 L** 等于**系统处理缺页的平均时间 S** 时， CPU 的利用率达到最大。

**当 L<S 时，表明系统频繁缺页**， CPU 利用率低，会导致系统抖动。

##### 利用缺页率发现抖动

下图是缺页率与进程分得物理块数之间的关系。当缺页率超过上限时会引起抖动，因此应增加分配给进程的物理块；此时每增加一个物理块，其缺页率明显降低；当进程缺页率达到下限值时，物理块的进一步增加对进程缺页率的影响不大。

![利用缺页率发现抖动](../img/利用缺页率发现抖动.jpeg)



##### 平均缺页频率

缺页率算法是一种**直接的控制抖动方法**。该方法要求为每页设一个使用位，当该页被访问时，相应位置 1 。同时设计一个计数器，记录自上次进程产生缺页以来进程执行的时间。

- 方法 1 ：设置一个阈值 F ，如最近两次缺页时间间隔小于 F 则分配一个物理块给该进程；否则淘汰使用位为 0 的页，并减少该进程的物理块数，同时将该进程的剩余页使用位重置为 0 。
- 方法 2 ：设置两个阈值，当缺页率达到上限值时为进程增加物理块，当缺页率达到下限值时减少进程的物理块。

缺页率算法的缺点：当进程由一个局部转移到另一个局部时，在原局部中的页面未移出内存之前，**连续的缺页会导致该进程在内存的页面迅速增加**，产生对内存请求的高峰。


- **`tᵢ`**: 第 `i` 次与第 `i+1` 次缺页之间的时间间隔  
- **`fᵢ`**: 第 `i` 次缺页的瞬时频率，计算公式为：  $f_i=\frac{1}{t_i}$

平均缺页频率：$F=\frac{\sum_{i=1}^{n} f_i}{n}$
...
- 减少进程启动时发生的大量页面错误
- 在引用之前预取进程需要的所有或部分页面
- 但是，如果预取的页面未被使用， I/O 和内存就会被浪费

#### 页面大小的选择

| 符号 | 含义 |
|------|------|
| $s$  | 进程的平均长度（总内存需求，单位：字节） |
| $p$  | 页面大小（单位：字节） |
| $e$  | 每个页表项的大小（单位：字节） |
...
$$
\text{页表长度} = \frac{s}{p}\\
\text{页表空间} = \frac{s}{p} \times e = \frac{se}{p}\\
\text{内部碎片} = \frac{p}{2}\\
f(p) = \frac{se}{p} + \frac{p}{2}\\
\frac{df}{dp} = -\frac{se}{p^2} + \frac{1}{2}\\
-\frac{se}{p^2} + \frac{1}{2} = 0 \implies \frac{se}{p^2} = \frac{1}{2} \implies p^2 = 2se\\
p = \sqrt{2se}
$$


$$
\boxed{p_{\text{opt}} = \sqrt{2 \times \text{(进程平均长度)} \times \text{(页表项大小)}}}
$$

页的大小总为 2 的幂。

页面尺寸多在 512B 到 16MB 之间。

#### TLB 范围

- TLB 范围：从 TLB 可访问的存储器数量
...
请求分段存储管理系统基于**分段存储**管理，是在分段存储管理系统的基础上，增加了**请求调段、分段置换**功能所形成的一种虚拟存储系统。

在请求分段存储管理中，作业运行之前，只要求将当前需要的若干个分段装入主存，便可启动作业运行。在作业运行过程中，若所要访问的分段不在主存，则通过调段功能将其调入，同时还可以通过**置换功能将暂时不用的分段换出到外存**上，以便腾出内存空间。

支持机构：段表、缺段中断机构、地址变换机构、请求调段和分段置换软件
...
| **段长** | - | 该段的长度(字节数) |
| **段基址** | - | 该段在内存中的起始物理地址 |
| **存取方式** | 2-3bit | 标识段的访问权限：<br>• `R` 读 <br>• `W` 写 <br>• `X` 执行 |
| **访问字段** | 8-16bit | 记录该段被访问的频率(用于置换算法) |
| **修改位** | 1bit | `1`=段内容被修改过(dirty)<br>`0`=未修改 |
| **存在位** | 1bit | `1`=段已在主存<br>`0`=段在外存 |
| **增补位** | 1bit | `1`=段可动态增长<br>`0`=固定长度 |
| **外存地址** | - | 该段在磁盘上的存储位置 |

1. **段号+段长+基址**​ 
   构成段式地址转换的基本三元组，通过`逻辑地址=(段号,段内偏移)`转换为`物理地址=基址+偏移`
...
![缺段中断处理流程](../img/缺段中断处理流程.png)

**内存管理：**请求分段的内存管理与与分区管理类似，采用连续内存管理方式。

**段的置换：**调入缺段时，若有足够的空闲分区则直接调入，否则查看空闲区之和能否满足要求，若能则进行拼接，否则进行置换。置换时可能要淘汰多段。

对于顺序执行程序，**缺页中断次数等于其访问的页帧数**。随着页面尺寸的增大，存放程序需要的页帧数就会减少，因此缺页中断的次数也会减少。

当缺页中断处理完成后，应该重新执行引起缺页中断的访存指令。

#### 请求分段的地址变换

![请求分段的地址变换过程](../img/请求分段的地址变换过程.png)

#### 共享段

在段式存储管理时，共享段表是用来实现多个进程共享同一段代码或数据的。用一个**单独的共享段表**来描述这些段，而不是在每个进程的段表中都保存一份，既**节省内存空间**，又**便于实现对共享段的更新和维护**。（但是对于普通的段表是每个进程一份的）,进程通过**私有段表中的共享标志位**和**共享段表的全局索引**协同确定内存访问路径。这种设计既隔离了进程私有内存，又高效支持共享段访问。

| 字段名       | 说明                          | 示例值              |
|--------------|-------------------------------|--------------------|
| **段名**     | 共享段的唯一标识符            | libc.so            |
| **段长**     | 该段的总字节数                | 0x8000 (32KB)      |
| **内存地址** | 段加载到内存的起始物理地址    | 0x400000           |
| **状态**     | 当前状态：<br>• LOADED(已加载)<br>• SWAPPED(已换出) | LOADED             |
| **外存地址** | 段在磁盘上的存储位置          | /lib/libc.so       |

| 字段名         | 说明                          | 特殊特性                  |
|----------------|-------------------------------|--------------------------|
| **共享计数COUNT**​ | 共享该段的进程总数            | 当COUNT=0时回收段资源    |
| **进程名**​     | 共享进程的名称                | chrome, firefox          |
| **进程号PID**​  | 共享进程的系统标识            | 1024, 2048               |
| **局部段号**​   | 该段在进程内的段号            | 不同进程中段号可以不同   |
| **存取控制**​   | 进程对段的操作权限：<br>• R(读)<br>• W(写)<br>• X(执行) | 不同进程可设不同权限     |

```mermaid
sequenceDiagram
    participant Process
    participant SharedSegTable
    participant MMU
    
    Process->>SharedSegTable: 请求加载共享段(libc.so)
    SharedSegTable-->>Process: 检查是否已记录该段
    alt 首次加载
        SharedSegTable->>Disk: 读取外存地址内容
        SharedSegTable->>Memory: 分配内存地址
        SharedSegTable->>SharedSegTable: 创建主表项(COUNT=1)
    else 已存在
        SharedSegTable->>SharedSegTable: COUNT++
    end
    SharedSegTable->>Process: 返回局部段号
    Process->>MMU: 通过局部段号访问
```

1. **进程请求加载共享段**
   进程向共享段表（SharedSegTable）发起请求.
2. **检查共享段是否已存在**
   共享段表检查该段是否已被其他进程加载：
   - 首次加载：若未记录，则执行以下操作：
     1. 从外存（Disk）读取共享段内容。
     2. 在内存（Memory）中分配物理地址空间。
     3. 在共享段表中创建主表项，初始化共享计数`count = 1`，并记录内存地址、存取控制等信息。
   - 已存在：若该段已被加载，则直接递增共享计数`count++`。
3. **返回局部段号**
   共享段表向进程返回一个局部段号（进程内唯一的标识），该段号可能与共享段在共享段表中的实际段号不同。
4. **进程通过MMU访问共享段**
   进程使用局部段号向内存管理单元（MMU）发起访问，MMU通过查询**进程段表**和**共享段表**完成地址转换，最终访问共享内存。
5. **共享段的回收**
   当进程不再需要共享段时：
   1. 撤销进程段表中对应的表项。
   2. 共享段表执行`COUNT--`。
   3. 若`COUNT=0`，则释放该段占用的内存，并删除共享段表中的表项；否则仅移除当前进程的记录
