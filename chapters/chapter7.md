...
> **举例说明（LRU 算法）**：
>
> - **初始状态（n = 3）**：内存中页面为 {A, B, C}（按访问时间排序：C 最新，A 最旧）。
> - 访问序列：D → B → E
>   - **n = 3 时**：调入 D 需淘汰 A → {B, C, D}；访问 B（更新顺序）→ {C, D, B}；调入 E 需淘汰 C → {D, B, E}。
>   - **n = 4 时**：调入 D → {A, B, C, D}；访问 B → {A, C, D, B}；调入 E → {A, C, D, B, E}（无需淘汰）。
> - **验证性质**：{D, B, E}（n = 3 的结果）是 {A, C, D, B, E}（n = 4 的结果）的子集。
>
...
2. 若第 1 步失败，则开始第 2 轮扫描，寻找R=0,U=1 的页面，将满足条件的第一个页面作为淘汰页，并将所有经历过页面的访问位置 0 。
...
> #### 1. 工作集窗口的定义
>
> **工作集窗口（Working Set Window, Δ）** 是一个**固定长度的虚拟时间窗口**，用于统计进程在最近Δ次内存访问中引用的页面集合。
>
> - **Δ的单位**：通常以**页面访问次数**（如最近1000次访存）或**实际时间**（如过去10ms）衡量。
> - **核心思想**：通过滑动窗口动态跟踪进程的**活跃页面集**，反映其当前内存需求。
>
> #### 2. 工作集窗口的核心作用
>
> ##### (1) 识别活跃页面（Locality Principle）
>
> - **问题**：进程的页面访问具有局部性，但局部性的范围随时间变化。
> - **解决方案:**
>   - 窗口Δ内的页面引用构成**工作集（Working Set）**，代表进程当前实际需要的页面。
>   - **示例**：若Δ=5，最近访问的页面序列为A→B→C→A→D，则工作集={A,B,C,D}。
>
> ##### (2) 指导页面置换（Prevent Thrashing）
>
> - **传统置换算法（如LRU）的缺陷**：可能淘汰未来仍要使用的页面（因局部性变化）。
> - 工作集策略：
>   - **保留工作集内所有页面**，仅置换窗口Δ外的页面（即长期未使用的“冷页”）。
>   - **示例**：若工作集={A,B,C}，内存满时优先淘汰不在该集合中的页面（如D）。
>
> ##### (3) 动态内存分配（Load Control）
>
> - **问题**：如何判断进程需要多少物理页框？
>
> - 解决方案：
>
>   - 工作集大小$|W(Δ)|$直接反映进程的**最小物理页框需求**。
> - 系统可据此动态分配内存（如：若$|W(Δ)|=4$，则至少分配4页给该进程）。
>
> #### 3. 工作集窗口的数学模型
>
> - **工作集定义**： $W(t,\Delta)=\{\text{Page p}|\text{p 在时间间隔}[t-\Delta t,t]\text{被访问}\}$
>
> - **工作集大小**：$|W(t, \Delta)|$表示进程在窗口Δ内的**活跃页面数量**。
>
> - 关键性质：
>
>   - 若Δ过小 → 工作集未覆盖完整局部性（低估需求）。
> - 若Δ过大 → 包含过多历史页面（高估需求）。
>
> #### 4. 窗口Δ的选取原则
>
> | **Δ的取值**           | **影响**                                                     |
> | --------------------- | ------------------------------------------------------------ |
> | **Δ过小**（如Δ=10）   | 工作集仅包含极短期页面，可能导致频繁缺页（未覆盖完整局部性）。 |
> | **Δ适中**（如Δ=1000） | 准确反映进程的活跃页面集，平衡内存占用与缺页率。             |
> | **Δ过大**（如Δ=∞）    | 工作集=所有历史访问页面，退化为全局置换策略（如FIFO），丧失局部性优化能力。 |
>
> **经验值**：Δ通常设置为进程平均局部性周期的2-3倍（需通过实验调优）。
>
> #### 5. 实例分析
>
> 假设进程访问序列如下（数字代表页面号）：
...
> ```
>
> - 取Δ=4：滑动窗口覆盖最近4次访问，工作集动态变化：
>
>   - 访问至`5,1,2,3`时，W={1,2,3,5}。
>   - 访问至`3,4,5`时，W={3,4,5}。
>   
> - **置换策略**：若内存仅能容纳3页，优先保留工作集内页面（如{3,4,5}），淘汰其他（如1,2）。
>
> #### 6. 工作集算法的实现优化
>
> - 近似实现：实际系统（如Windows/Linux）不会严格维护Δ次历史记录，而是：
>   - **时钟算法（Clock Algorithm）**：通过访问位近似模拟工作集。
>   - **老化计数器（Aging Counters）**：定期衰减页面权重，模拟滑动窗口。
> - **硬件支持**：MMU的访问位（Reference Bit）可快速检测页面活跃性。
>
> ### 总结
>
> 工作集窗口Δ通过**限定统计范围**，实现了：
>
> 1. **动态捕捉局部性**：避免静态置换策略的盲目性。
> 2. **预防系统颠簸**：确保进程所需页面常驻内存。
> 3. **资源分配依据**：为内存调度提供量化指标。
>
> 其本质是**在时间维度和空间维度间寻找平衡**，是操作系统内存管理的核心理论之一。
