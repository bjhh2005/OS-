...
- 别名：高级调度、作业调度、接纳调度。
- 功能：
  - 决定哪些作业（Job）从外存（后备队列）调入内存，并为其创建进程。
  - 控制系统的多道程序度（Degree of Multiprogramming），即内存中同时运行的进程数量。
  - 运行频率最低（几分钟一次），调度开销较大。
- 影响：
  - 若调度过于激进（进程过多），可能导致内存紧张；过于保守则降低系统吞吐量。
  - 批处理系统必备，但分时/实时系统通常不需要（用户直接交互，无需作业队列）。

#### 中程调度（Medium-Term Scheduling）

- 别名：内存调度、交换调度。
- 功能：
  - 管理进程在内存与外存（交换区）间的换入（Swap In）和换出（Swap Out）。
  - 当内存不足时，将阻塞或低优先级进程换出，释放资源；内存空闲时再换入。
  - 运行频率适中，介于长程与短程调度之间。
- 目的：提高内存利用率，平衡系统负载（如避免I/O密集型进程长期占用内存）。

#### 短程调度（Short-Term Scheduling）

- 别名：进程调度、低级调度、CPU调度。
- 功能：
  - 从内存的就绪队列中选择一个进程，分配CPU执行权。
  - 运行频率最高（毫秒级），直接决定CPU的利用率与响应时间。
  - 支持抢占式（如时间片轮转）或非抢占式（如FCFS）调度。
- 关键算法：
  - 先来先服务（FCFS）、短作业优先（SJF）、优先级调度、多级反馈队列等
...
4. **带权周转时间：**$\frac{周转时间}{服务时间}$。
5. **等待时间：** 衡量任务在就绪队列中等待的总时间。
6. **响应时间：** 从任务提交到首次开始执行所经过的时间



### 经典的单核进程调度算法

对于用户级线程来说，每个进程可以定制自己的调度算法，线程管理比较灵活。而对内核级线程，**调度算法由内核决定，用户无法修改**。
...
> 特别注意，进程中设置内核级线程和用户级线程的效果完全不同。
>
> 1. **内核级线程 (KLT) 决定调度**：无论进程有多少 ULT，内核只根据 KLT 数量分配时间片。
> 2. **用户级线程 (ULT) 仅影响进程内部并发**：ULT 的多少不影响进程的总 CPU 时间，仅影响如何利用已分配的时间片。
>
> 具体地讲，
>
> 1. **用户级线程**的调度仍以**进程**为单位，各个进程轮流执行一个时间片，假设进程A包含1个用户线程，进程B包含100个用户线程，那么A中单个线程运行时间将是B中各线程平均运行时间的100倍。
> 2. **内核级线程**的调度是以**线程**为单位的，各个线程轮流执行一个时间片，如果进程A包含1个内核级线程，进程B包含100个内核级线程，此时B的运行时间将是A的100倍。
...
IO繁忙型作业优于计算繁忙型作业，系统进程优于用户进程，与长作业、短作业或系统资源要求多少没有必然关系。
